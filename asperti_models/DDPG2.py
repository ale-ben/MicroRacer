...
def get_actor():
    ...
    out = layers.LayerNormalization()(out)
    outputs = layers.Dense(num_actions, name="perturbable3",
                           activation="tanh")(out)
...
def get_perturbed_actor_updates(actor, perturbed_actor, param_noise_stddev):
    updates = []
    for var, perturbed_var in zip(actor.trainable_variables,
                                  perturbed_actor.trainable_variables):
...
def adapt_param_noise(actor, adaptive_actor, states, current_stddev):
    ...
    perturb_adaptive_policy_ops = get_perturbed_actor_updates(
        actor, adaptive_actor, current_stddev)
    distance = tf.sqrt(
        tf.reduce_mean(tf.square(actor(states) - adaptive_actor(states))))
...
perturb_policy_ops = get_perturbed_actor_updates(actor_model,
                                                 param_noise_actor,
                                                 param_noise_stddev)
...
perturb_adaptive_policy_ops = get_perturbed_actor_updates(
    actor_model, adaptive_param_noise_actor, param_noise_stddev)
    ...
    while i < total_iterations:
        ...
        while not (done):
            ...
            if buffer.buffer_counter > batch_size:
                ...
                targetQ = rewards + (1 - dones) * gamma * (target_critic(
                    [newstates, target_actor(newstates)]))
                ...
                current_stddev = adapt_param_noise(actor_model,
                                                   adaptive_param_noise_actor,
                                                   states, current_stddev)
            prev_state = state
        ...
        perturb_policy_ops = get_perturbed_actor_updates(
            actor_model, param_noise_actor, current_stddev)
