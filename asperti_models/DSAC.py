...
class Get_actor(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # action = dist.sample()
        action = mu + sigma * tfp.distributions.Normal(0,
                                                       1).sample(num_actions)
...
if load_weights:
    target_critic(
        [layers.Input(shape=(num_states)),
         layers.Input(shape=(num_actions))])
...
def update_critics(states, actions, rewards, dones, newstates):
    ...
    q_hat = tf.clip_by_value(q_hat,
                             clip_value_min=q - clip_boundary,
                             clip_value_max=q + clip_boundary)
    ...
    critic1_gradient = tape1.gradient(loss_c1,
