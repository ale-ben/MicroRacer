...
def borders(cs, theta, track_semi_width=0.02):
    d = cs(theta, 1)
    dx = d[:, 0]
    dy = d[:, 1]
    # print(dx, dy)
...
def create_random_track(curves=20, track_width=0.04, chicanes=False):
...
def no_inversion(thetanew, thetaold):
    if thetaold < -np.pi * 0.9 and thetanew > np.pi * 0.9:
...
def obs_points(csout, theta, track_width=0.04, obs_length=np.pi / 150):
    obs_side = np.random.choice([track_width * 0.85, track_width * 0.15],
                                size=len(theta))
    d = csout(theta, 1)
    dx = d[:, 0]
    dy = d[:, 1]
...
def generate_obstacles(csout, obstacles_n, map, track_width):
    ...
    theta = np.random.uniform(
        low=(2 * np.pi) / (obstacles_n * 2),
        high=(2 * np.pi) / obstacles_n,
        size=obstacles_n,
    )
...
class Racer:
    ...
    def reset(self, shared_map=None):
        if shared_map == None:
            ...
            while not (legal_map):
                self.cs, self.csin, self.csout = create_random_track(
                    self.curves, self.track_width, chicanes=self.chicanes)
                self.map, legal_map = create_route_map(self.csin, self.csout)
                if self.obstacles:
                    _, self.obs_pos = generate_obstacles(
                        self.csout, self.obstacles_number, self.map,
                        self.track_width)
                else:
                    self.obs_pos = []
        else:
        ...
        if shared_map == None:
            if self.low_speed_termination:
                v = max(np.random.uniform() * 0.5, 0.07)
            else:
        ...
        self.sinit_v = v
        vnorm = v / ((self.carvx**2 + self.carvy**2)**0.5)
        self.carvx *= vnorm
        self.carvy *= vnorm
        assert self.map[int(self.carx * 500) + 650, int(self.cary * 500) + 650]
        lidar_signal = lidar_grid(self.carx, self.cary, self.carvx, self.carvy,
                                  self.map)
    ...
    def step(self, action):
        ...
        acc, turn = action
        v = (self.carvx**2 + self.carvy**2)**0.5
        ...
        if on_route and no_inversion(newcartheta, self.cartheta):
            ...
            lidar_signal = lidar_grid(self.carx, self.cary, self.carvx,
                                      self.carvy, self.map)
...
def newrun(actors,
    ...
    for i, actor in enumerate(actors):
        ...
        if i == 0:
            ...
            states.append(racers[i].reset(
                (racers[0].cs, racers[0].map, racers[0].obs_pos,
                 racers[0].sinit_v)))
    ...
    def animate(i):
        ...
        for lnum, line in enumerate(lines):
            if lnum < len(actors):
                ...
                if not racers[lnum].done:
                    ...
                    L.get_texts()[lnum].set_text(
                        "{}° place".format(podium.index(lnum) + 1))
        ...
        if not racers[0].done and debug_dir:
            single_s = states[0]
            dist_lid = single_s[2]
            single_s = single_s[0] + np.arctan2(racers[0].carvy,
                                                racers[0].carvx)
            lines[-3].set(
                xdata=[
                    racers[0].carx,
                    racers[0].carx + np.cos(single_s) *
                    (dist_lid /
                     ((np.cos(single_s)**2 + np.sin(single_s)**2)**0.5)),
                ],
                ydata=[
                    racers[0].cary,
                    racers[0].cary + np.sin(single_s) *
                    (dist_lid /
                     ((np.cos(single_s)**2 + np.sin(single_s)**2)**0.5)),
                ],
            )
            min_angle = (np.arctan2(racers[0].carvy, racers[0].carvx) -
                         racers[0].max_turnrad)
            lines[-2].set(
                xdata=[racers[0].carx, racers[0].carx + np.cos(min_angle)],
                ydata=[racers[0].cary, racers[0].cary + np.sin(min_angle)],
            )
            max_angle = racers[0].max_turnrad + np.arctan2(
                racers[0].carvy, racers[0].carvx)
            lines[-1].set(
                xdata=[racers[0].carx, racers[0].carx + np.cos(max_angle)],
                ydata=[racers[0].cary, racers[0].cary + np.sin(max_angle)],
            )
...
def metrics_run(
    ...
    for ep in range(runs_num):
        ...
        if racer.completation == 1:
            completation += 1
        print("Episode {}: Steps = {}, Ep. reward = {}. Avg. Ep. speed = {}".
              format(ep, steps, episodic_reward, mean_speed / steps))

    print("Completed episodes: {}/{}".format(completation, runs_num))
    print("Avg reward over {} episodes = {} / Avg steps = {} / Avg speed = {}".
          format(
              runs_num,
              np.mean(ep_reward_list),
              np.mean(ep_steps_list),
              np.mean(ep_meanspeed_list),
          ))
    print("\n")
